...
- **Status**: Proposta
- **Data**: 2026-08-31
- **Impacta**: `channel`, onboarding, UX de recibo, roadmap (encerra decisão
  aberta #1, remove risco declarado na ADR-0002)

## Contexto

A ADR-0002 registrou 1:1 como **mitigação de risco técnico**: `[a verificar]`
se a WhatsApp Cloud API opera em grupo, e recomendou desenhar a UX como 1:1
"tratando grupo como bônus e não como premissa" só para o caso de a resposta
ser não. Essa verificação segue como decisão aberta #1, bloqueando a Etapa
6-7.
...
Isso encerra a decisão aberta #1: a resposta de se a Cloud API opera em grupo
deixa de importar para a arquitetura, porque grupo nunca seria usado de
qualquer forma. Também remove o risco declarado na ADR-0002 — não há UX
...
- Fecha a decisão aberta #1 sem depender de verificação externa na
  documentação da Meta — deixa de ser bloqueio de roadmap.
- Remove o risco de redesenho de UX declarado na ADR-0002: não existe UX
