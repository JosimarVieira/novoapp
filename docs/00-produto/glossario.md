...
| Curto-circuito | — (regra 6 do `CLAUDE.md`) | Resolver `sim`, `não`, `desfazer` ou um número sem chamar o modelo. Resposta que não é nenhum desses vai ao modelo, com ou sem pergunta em aberto — o curto-circuito cobre os atalhos, não toda resposta. |
| Pendência superada | `PendingAction.expiresAt` | Pergunta que deixou de esperar resposta porque a pessoa mudou de assunto e a mensagem nova venceu ([ADR-0029](../01-adr/0029-intencao-adiada-e-precedencia-de-mensagem-nova.md)). Não é rejeitada — ninguém disse não: `resolution` continua nula e ela segue na Central de pendências, igual às que venceram por tempo. |
| Correção livre | `ConfirmSuggestedCategoryTool` | Resposta a uma pergunta de criação de categoria que não é `sim`, `não` nem `desfazer` — "restaurante dentro de alimentação". Não é erro nem repetição da pergunta: é a terceira via, que corrige nome e hierarquia de uma vez e pode criar categoria-pai e subcategoria juntas. Lida pela mesma e única chamada ao modelo que qualquer outra resposta livre a uma pergunta em aberto — a tool dela entra no cardápio só quando há categoria oferecida a corrigir ([ADR-0026](../01-adr/0026-hierarquia-na-criacao-de-categoria-por-chat.md), [ADR-0029](../01-adr/0029-intencao-adiada-e-precedencia-de-mensagem-nova.md)). |
